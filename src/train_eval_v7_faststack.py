# train_eval_v7_faststack.py
# Faster hierarchical categorizer with batched prediction, progress bars, and
# configurable parallelism. Uses word+char TF-IDF (float32), a coarse router
# with brand boosts, and per-coarse fine ensembles (LR+CNB).
#
# v7 changes vs v6:
# - Vectorized F1 threshold search (much cheaper, still supports joblib parallel).
# - Optional disabling of explanation strings (big perf win when off).
# - Slightly safer defaults for parallelism knobs.
# - Cleaned up a few small hotspots and edge cases.
...
try:
    from featurizer_patch import normalize_text  # type: ignore
except ModuleNotFoundError:
    from src.featurizer_patch import normalize_text  # type: ignore

try:
    from lexicon_patch import COARSE_KEYS, coarse_boost, fine_hard_override  # type: ignore
except ModuleNotFoundError:
    from src.lexicon_patch import COARSE_KEYS, coarse_boost, fine_hard_override  # type: ignore
...
def build_text_union_fast(word_max: int, char_max: int, word_min_df: int = 2, char_min_df: int = 2):
    """Word + char TF-IDF, float32, tuned for short financial strings."""
...
def transform_in_chunks(vec, texts: List[str], chunk_size: int, desc: str) -> csr_matrix:
    """
    Chunked transform to avoid spikes in RAM.
    Kept sequential on purpose; sklearnex/OpenMP will
    still use multiple cores inside transform.
    """
    mats = []
    n = len(texts)
    ...
    return vstack(mats, format='csr') if len(mats) > 1 else mats[0]


def predict_proba_in_chunks(model, X, chunk_size: int, desc: str) -> np.ndarray:
    """
    Chunked predict_proba – trades a tiny bit of overhead
    for much more stable memory behaviour.
    """
    parts = []
    n = X.shape[0]
    for i in tqdm(range(0, n, chunk_size), desc=desc):
...
FINE_MIN = {
    ...
    'SHOPPING_ECOM': 0.30, 'SHOPPING_ELECTRONICS': 0.18, 'ENTERTAINMENT': 0.20,
...
def _augment_hierarchy(base_map: Dict[str, List[str]],
                       fines: List[str]) -> Tuple[Dict[str, List[str]], Dict[str, str], List[str]]:
    """Ensure every fine label is attached to some coarse bucket."""
...
# ----------------------------
# Threshold calibration (vectorized)
# ----------------------------
def _best_threshold_vec(y_binary: np.ndarray, p: np.ndarray,
                        tgrid: np.ndarray) -> float:
    """
    Vectorized search over tgrid for best F1 on a single one-vs-rest class.
    y_binary: 0/1 array, p: predicted probs for that class.
    """
    # Ensure 0/1 ints
    y = (y_binary > 0).astype(np.int8)
    if y.ndim != 1:
        y = y.ravel()
    if p.ndim != 1:
        p = p.ravel()

    # Broadcast: S[t, i] = (p_i >= t)
    S = p[None, :] >= tgrid[:, None]
    ...
    TP = (S & y1).sum(axis=1)
    FP = (S & not_y1).sum(axis=1)
    FN = ((~S) & y1).sum(axis=1)

    # Avoid division by zero
    denom_prec = np.maximum(TP + FP, 1e-12)
    denom_rec = np.maximum(TP + FN, 1e-12)

    precision = TP / denom_prec
    recall = TP / denom_rec
    denom_f1 = np.maximum(precision + recall, 1e-12)
    f1 = 2.0 * precision * recall / denom_f1

    best_idx = int(f1.argmax())
    return float(tgrid[best_idx])


def compute_class_thresholds(y_true, proba, classes, min_floor=None):
    """
    Sequential per-class threshold search, but using vectorized F1 internally.
    Suitable for small number of classes.
    """
    thresholds = {}
    tgrid = np.linspace(0.05, 0.95, 101, dtype=np.float32)

    for i, cls in enumerate(classes):
        y = (y_true == cls).astype(np.int8)
        p = proba[:, i]
        if len(np.unique(y)) < 2:
            best_t = 0.50
        else:
            best_t = _best_threshold_vec(y, p, tgrid)
        if min_floor and cls in min_floor:
            best_t = max(best_t, float(min_floor[cls]))
        thresholds[cls] = float(best_t)
    return thresholds


def compute_class_thresholds_parallel(y_true, proba, classes,
                                      min_floor=None, n_jobs: int = 1):
    """
    Parallel per-class threshold search. Uses the same vectorized inner
    routine, but splits across classes using joblib.
    """
    tgrid = np.linspace(0.05, 0.95, 101, dtype=np.float32)
    ...
    def _one(i_cls):
        ...
        if len(np.unique(y)) < 2:
            ...
            thr = _best_threshold_vec(y, p, tgrid)
        if min_floor and cls in min_floor:
            thr = max(thr, float(min_floor[cls]))
        return (cls, float(thr))

    items = list(enumerate(classes))
    if n_jobs and n_jobs != 1:
        out = Parallel(n_jobs=n_jobs)(
            delayed(_one)(it) for it in tqdm(items, desc='Calibrate thresholds')
        )
    else:
...
def _get_feature_names(vec) -> List[str]:
    try:
        ...
        try:
            ...
            names = list(getattr(w, 'get_feature_names_out', lambda: [])()) + \
                    list(getattr(c, 'get_feature_names_out', lambda: [])())
        except Exception:
            pass
        return names
...
def build_fine_classifier(n_jobs: int = -1):
    """
    Fine-level ensemble: LR + ComplementNB with soft voting.
    n_jobs propagates into LR; NB is single-threaded and cheap.
    """
...
def main():
    ap = argparse.ArgumentParser(description='v7 fast hierarchical categorizer')
    # Featurizer knobs
    ...
    # Global parallelism knobs
    ap.add_argument('--n-jobs', type=int, default=-1,
                    help='Default n_jobs for LR and ensembles (-1=all cores)')
    ...
    ap.add_argument('--chunk-size', type=int, default=25_000,
                    help='Batch size for vectorizer transform and predict')

    # Task-level parallelism
    ap.add_argument('--bucket-workers', type=int, default=-1,
                    help='Parallel workers for training fine buckets '
                         '(-1=all cores, 1=sequential)')
    ap.add_argument('--predict-workers', type=int, default=-1,
                    help='Parallel workers for per-bucket prediction '
                         '(-1=all cores, 0=sequential)')
    ap.add_argument('--threshold-workers', type=int, default=-1,
                    help='Parallel jobs for per-class threshold search '
                         '(-1=all cores, 0=sequential)')

    # Coarse trainer
    ap.add_argument('--coarse-trainer', choices=['lr', 'sgd'], default='lr',
                    help='Coarse router trainer: LogisticRegression or SGD (log loss)')
    ...
    ap.add_argument('--coarse-batch', type=int, default=10_000)

    # Explanations / review
    ap.add_argument('--enable-explanations', action='store_true',
                    help='If set, compute LR feature attributions for low-conf predictions')
    ap.add_argument('--disable-review', action='store_true',
                    help='If set, do not write data/review.csv low-confidence file')
    ...
    ENABLE_EXPL = bool(args.enable_explanations)
    ...
    X_train = transform_in_chunks(
        vec, train_df['raw_text'].tolist(),
        args.chunk_size, desc='Vectorize train'
    )
    X_test = transform_in_chunks(
        vec, test_df['raw_text'].tolist(),
        args.chunk_size, desc='Vectorize test'
    )
    ...
    COARSE_TO_FINE, FINE_TO_COARSE, FINE_LABELS = _augment_hierarchy(
        BASE_COARSE_TO_FINE,
        sorted(list(set(y_fine_train) | set(y_fine_test)))
    )

    y_coarse_train = np.array(
        [FINE_TO_COARSE.get(lbl, 'OTHER_MISC') for lbl in y_fine_train]
    )
    y_coarse_test = np.array(
        [FINE_TO_COARSE.get(lbl, 'OTHER_MISC') for lbl in y_fine_test]
    )
    ...
    def fit_coarse_router():
        if args.coarse_trainer == 'sgd':
            ...
            for ep in range(1, args.coarse_epochs + 1):
                ...
                rng.shuffle(idx_all)
                for s in tqdm(
                    range(0, len(idx_all), args.coarse_batch),
                    desc=f'Coarse SGD epoch {ep}/{args.coarse_epochs}'
                ):
            ...
            tqdm.write(
                f'Fitting LogisticRegression(saga) '
                f'max_iter={args.coarse_max_iter} tol={args.coarse_tol} '
                f'n_jobs={args.n_jobs}'
            )
            lr.fit(X_train, y_coarse_idx,
                   sample_weight=train_df['__weight__'].values)
            return lr
    ...
    rng_coarse = np.random.RandomState(42)
    cal_size = min(args.coarse_cal_size, len(train_df) // 5)
    cal_idx = rng_coarse.choice(len(train_df), size=cal_size, replace=False)

    X_cal = X_train[cal_idx]
    y_cal = y_coarse_train[cal_idx]
    ...
    if args.threshold_workers and args.threshold_workers != 0:
        coarse_thresholds = compute_class_thresholds_parallel(
            y_true=y_cal,
            proba=coarse_proba_cal,
            classes=coarse_classes_keys,
            min_floor=COARSE_MIN,
            n_jobs=args.threshold_workers
        )
    else:
    ...
    inner_jobs = args.n_jobs if args.n_jobs not in (0, None) else 1
    ...
    def train_bucket(coarse_key: str):
        ...
        uniq = np.unique(yk)
        if uniq.size < 2:
            # Only one fine label in this bucket – trivial router
            solo_lbl = uniq[0]
            thr_map = {solo_lbl: FINE_MIN.get(solo_lbl, 0.20)}
            return (coarse_key, None, None, thr_map, solo_lbl)
        ...
        # Bucket-specific fine thresholds
        ncal = min(2000, max(200, int(args.fine_cal_frac * Xk.shape[0])))
        rng_f = np.random.RandomState(123)
        cal_sel = rng_f.choice(Xk.shape[0], size=ncal, replace=False)
        ...
        thr_k = compute_class_thresholds(
            y_true=y_cal_k,
            proba=prob_cal,
            classes=classes_k,
            min_floor=FINE_MIN
    ...
    if args.bucket_workers and args.bucket_workers != 1:
        results = Parallel(
            n_jobs=args.bucket_workers,
            prefer='threads'
        )(
            delayed(train_bucket)(k)
            for k in tqdm(keys, desc='Dispatch fine buckets')
        )
    else:
    ...
    coarse_proba = predict_proba_in_chunks(
        coarse_lr, X_test, args.chunk_size, desc='Coarse routing'
    )
    ...
    def predict_bucket(ck: str, idxs: List[int]):
        ...
        if ck not in fine_models or fine_models[ck] is None:
            # Fallback – if no fine model for this bucket
            for i in idxs:
        ...
        # Optional: explanation only when enabled
        named_estimators = getattr(model, 'named_estimators_', {})
        lr_part = None
        if ENABLE_EXPL and isinstance(named_estimators, dict):
            lr_part = named_estimators.get('lr', None)
        ...
        for row_i, proba in zip(idxs, probas):
            ...
            expl = ""
            if ENABLE_EXPL and lr_part is not None and pmax < LOW_CONF_ROUTE:
                try:
                    xrow = X_test[row_i]
                    # Align LR coef row with encoded class index
                    class_idx_lr = getattr(lr_part, 'classes_', None)
                    if class_idx_lr is None:
                        coef = lr_part.coef_[j]
                    else:
                        # map j (Voting column position) -> position in LR classes
                        if j < len(class_idx_lr):
                            coef = lr_part.coef_[j]
                        else:
                            coef = lr_part.coef_[0]
    ...
    if args.predict_workers and args.predict_workers != 0:
        par_out = Parallel(
            n_jobs=args.predict_workers,
            prefer='threads'
        )(
            delayed(predict_bucket)(ck, idxs)
            for ck, idxs in tqdm(items, desc='Dispatch predict buckets')
        )
        flat = [t for part in par_out for t in part]
    else:
        flat: List[Tuple[int, str, float, str]] = []
        for ck, idxs in tqdm(items, desc='Predict fine buckets'):
            flat.extend(predict_bucket(ck, idxs))

    # assemble in original order
    n_test = len(test_df)
    y_pred: List[str] = [None] * n_test  # type: ignore
    y_prob: List[float] = [0.0] * n_test
    y_expl: List[str] = [""] * n_test
    ...
    pred_seconds = max(1e-6, t_pred_end - t_pred_start)
    throughput = n_test / pred_seconds
    latency_ms = (pred_seconds / n_test) * 1000.0
    ...
    print(f"Throughput (hierarchy only): {throughput:,.1f} tx/s")
    print(f"Latency (hierarchy only): {latency_ms:.3f} ms/tx")
    ...
    with open(os.path.join(REPORT_DIR, 'metrics.json'), 'w', encoding='utf-8') as fh:
        json.dump({
            ...
            'n_test': int(n_test),
            'throughput_tx_per_s': float(throughput),
            'latency_ms_per_tx': float(latency_ms),
        }, fh, indent=2)

    # Feedback routing (optional)
    if not args.disable_review:
        ...
        if review_mask.any():
            ...
            out['explain'] = y_expl
            out.loc[review_mask, ['raw_text', 'label', 'pred', 'pred_proba', 'explain']] \
    ...
    if args.save_model:
        try:
            ...
            }, mdl_dir / 'txcat_fastcpu_v7.joblib')
            print('Saved models/txcat_fastcpu_v7.joblib')
