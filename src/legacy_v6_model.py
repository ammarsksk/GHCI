...
def main():
    ...
    # -1 means "all cores" for joblib / sklearn
    ap.add_argument('--bucket-workers', type=int, default=-1, help='Parallel workers for training fine buckets (outer parallelism, -1=all cores, 1=sequential)')
    ap.add_argument('--predict-workers', type=int, default=-1, help='Parallel workers for per-bucket prediction (-1=all cores, 0=sequential)')
    ...
    ap.add_argument('--threshold-workers', type=int, default=-1, help='Parallel jobs for per-class threshold search (-1=all cores, 0=sequential)')
    args = ap.parse_args()
    ...
    # Use requested core count for fine classifiers as well
    ...
    # If bucket_workers is 0 or 1, run sequentially; any other value (including -1) uses Parallel.
